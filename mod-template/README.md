...
- Set top-level `"runAt"` in `config.json` to control injection timing: `"document_start"`, `"document_end"` (default), or `"document_idle"`. `"document_start"` runs before the page's scripts and before the DOM is built — guard any DOM access.
